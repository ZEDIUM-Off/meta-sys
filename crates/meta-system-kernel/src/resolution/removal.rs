//! Private cleanup plan for removing a Component Instance and affected Bindings.
...
use crate::{
    runtime::KernelError,
    system::{
        ComponentDefinition, ComponentDefinitionId, ComponentInstanceId, ComponentRuntime, Context,
        ContextId, ContextOwner, Effect, EffectId, Facet, FacetId, FacetTarget, GraphState,
        ResolutionState,
    },
};
...
impl GraphState {
    ...
    pub(crate) fn apply_removal(&mut self, plan: &RemovalPlan) {
    ...
    fn bindings_for_definition(&self, definition_id: ComponentDefinitionId) -> Vec<Binding> {
        self.definitions
            .get(&definition_id)
            .into_iter()
            .flat_map(ComponentDefinition::requirements)
