//! Implementation of the ordered Loader state machine.

use std::collections::BTreeMap;

use super::{
    ComponentMaterializer, LoadId, LoadPhase, LoadRecord, LoadRequest, Loader, LoaderDecision,
    LoaderError, LoaderEvent, LoaderHook, LoaderOutcome, LoaderProposal,
};
use crate::{
    execution::EventLoopDriver,
    runtime::{KernelEvent, KernelRuntime},
};
...
impl<Materializer: ComponentMaterializer> Loader<Materializer> {
    ...
    fn declare(&mut self, request: &LoadRequest) -> Result<LoaderOutcome, LoaderError> {
        ...
        let transition = super::LoadTransition::new(id, None, LoadPhase::Declared);
